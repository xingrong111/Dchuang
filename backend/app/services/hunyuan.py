# ============================================================
# 智绘锡承 - 腾讯混元 AI Provider（接入 Hunyuan3DService）
# 位置: backend/app/services/hunyuan.py
#
# 定位:
#   - HunyuanService 是面向现有 AI Provider 架构（BaseAIService）的统一实现，
#     内部组合 Hunyuan3DService SDK 封装层（腾讯云 AI3D 产品）
#   - 职责边界与 GLMService 一致: Route(ai.py) → get_ai_service() →
#     HunyuanService.generate_3d/query_status → Hunyuan3DService(SDK 封装)
#
# 当前能力：
#   - Provider 注册、工厂创建、凭据校验和请求构造链完整
#   - 通过 SubmitHunyuanTo3DProJob / QueryHunyuanTo3DProJob 真实提交与查询
#   - 是否调用腾讯 API 由 AI_PROVIDER 和调用入口显式控制
#   - analyze_style 由 GLM Provider 负责，混元明确不支持
#
...
class HunyuanService(BaseAIService):
    ...
    def __init__(self):
        ...
        # 复用 SDK 封装服务（凭据已校验；Client 惰性构造，不联网）
        self._sdk = Hunyuan3DService(
            secret_id=self.secret_id, secret_key=self.secret_key
        )
        # model 元数据 —— 反映实际生成模型（HUNYUAN_3D_MODEL 配置或 hunyuan-3d）。
    ...
    def generate_3d(self, task):
        """3D 生成（文生3D / 图生3D）——真实提交

        调用链：参数校验 → SDK 请求构造 → submit_job 真实提交
    ...
    def query_status(self, task):
        """任务状态查询（真实 QueryHunyuanTo3DProJob）
